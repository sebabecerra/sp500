...
function fitTwoFactorRegression(samples) {
  ...
  for (const sample of samples) {
    const row = [1, sample.x1, sample.x2];
    for (let i = 0; i < 3; i += 1) {
  ...
  const matrix = xtx.map((row, i) => [...row, xty[i]]);
  ...
  return {
    intercept: matrix[0][3],
    betaLevel: matrix[1][3],
    betaScore: matrix[2][3],
  };
}
...
function buildAnnualizedCurrentYear(monthly) {
  const latestYear = Math.max(...monthly.map((entry) => entry.year));
  const currentYearEntries = monthly
    .filter((entry) => entry.year === latestYear)
    .sort((a, b) => a.month - b.month);

  if (currentYearEntries.length === 0) return null;

  const prevDecember = monthly.find((entry) => entry.year === latestYear - 1 && entry.month === 12);
  const latestEntry = currentYearEntries.at(-1);

  if (!prevDecember || !latestEntry) return null;

  const monthsElapsed = latestEntry.month;
  if (monthsElapsed <= 0) return null;

  const ytdGrowth = latestEntry.amount / prevDecember.amount;
  const annualizedReturnPct = (Math.pow(ytdGrowth, 12 / monthsElapsed) - 1) * 100;

  return {
    year: latestYear,
    returnPct: annualizedReturnPct,
    bucket: bucketFor(annualizedReturnPct),
    annualized: true,
    monthsElapsed,
  };
}
